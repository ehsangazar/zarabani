...
function Copy({ from, to = from }: { from: number; to?: number }) {
  return <>{source.slice(from, to + 1).map((text, i) => <p key={from + i}>{text}</p>)}</>
}
...
function Cards({ items }: { items: [string, string, string][] }) {
  return <div className="ae-cards">{items.map(([symbol, title, text]) => <section key={title}><span className="ae-symbol" aria-hidden="true">{symbol}</span><h3>{title}</h3><p>{text}</p></section>)}</div>
}

function Table({ headings, rows }: { headings: string[]; rows: string[][] }) {
  return <div className="ae-table" tabIndex={0} role="region" aria-label={headings.join(', ')}><table><thead><tr>{headings.map(h => <th key={h} scope="col">{h}</th>)}</tr></thead><tbody>{rows.map((row, i) => <tr key={i}>{row.map((cell, j) => j === 0 ? <th scope="row" key={j}>{cell}</th> : <td key={j}>{cell}</td>)}</tr>)}</tbody></table></div>
}
...
export default function AltrataEditorial({ id, children, ImageSpace }: { id: string; children: ReactNode; ImageSpace: ComponentType<{ after: number }> }) {
  ...
  switch (id) {
    ...
    case 'discovery':
      ...
      break
    case 'workflow':
      content = <><Copy from={56} /><div className="ae-process"><div><span>01</span><h3>Start broad</h3><p>Inspect results and progressively refine the target.</p></div><div><span>02</span><h3>Hit the logic limit</h3><p>Alternatives and exclusions cannot be expressed in one query.</p></div><div><span>03</span><h3>Leave the product</h3><p>Export, merge and finish the filtering elsewhere.</p></div></div><Table headings={['Workaround', 'What clients had to do']} rows={[[source[59], source[60]], [source[61], source[62]], [source[63], source[64]]]} /><Takeaway>{source[66]}</Takeaway><Copy from={67} /></>
      break
    case 'scope':
      content = <><Copy from={70} to={71} /><Cards items={[
      ...
      ]} /><Table headings={['Decision', 'Immediate effect', 'Longer-term implication']} rows={[
      ...
      ]} /><Copy from={76} to={77} /><Takeaway>{source[82]}</Takeaway><Copy from={80} /></>
      break
    case 'constraints':
      content = <><Copy from={85} /><Table headings={['Boundary', 'What we established', 'Design implication']} rows={[
        ['Search engine', 'AND, OR and NOT already supported', 'Represent existing power in an approachable interface'],
        ['Cross-module logic', 'Not technically limited to Location', 'Keep relationships across Role, Location and Experience open'],
        ['Nesting', 'Unlimited nesting required backend changes outside the delivery window', 'Keep the logic shallow and understandable'],
        ['Everyday searches', 'Existing behaviour had to remain intact', 'Reveal expressive controls when they are needed'],
        ['Saved searches', 'The schema stored a flat set of filters', 'Capture persistence as a later dependency; prioritise creation, editing and execution'],
      ]} /><Copy from={91} /><Takeaway>{source[100]}</Takeaway></>
      break
    case 'concepts':
      content = <><Copy from={106} to={107} /><div className="ae-split ae-concepts"><section><Label>Concept A · Dedicated logic builder</Label><h3>Make the query structure explicit.</h3><div className="ae-blocks" aria-label="Illustrative builder structure"><span>Condition</span><b>AND / OR</b><span>Condition</span></div><p>Users moved from the filter panel into a structured builder, with explicit conditions, connectors and exclusions.</p><h4>What I was testing</h4><p>{source[120]}</p><h4>The trade-off</h4><p>Greater visibility and control introduced a second search model: when to enter it, how it related to the panel, and how to interpret the structure.</p></section><section><Label>Concept B · Inline Boolean controls</Label><h3>Extend the model users already know.</h3><div className="ae-blocks" aria-label="Illustrative inline control"><span>CEO</span><b>OR</b><span>CTO</span></div><p>AND / OR sat beside modules, filters and multiple values; NOT appeared at value level after selection. Proximity and chunking kept logic close to its subject.</p><h4>What I was testing</h4><p>{source[135]}</p><h4>The trade-off</h4><p>Familiarity supported simple searches, but complex query relationships became distributed across the panel.</p></section></div><ImageSpace after={135} /><h3>Four things would decide the direction</h3><div className="ae-capabilities"><span>Familiarity</span><span>Logic verification</span><span>Complexity at scale</span><span>Confidence in results</span></div><h3>I prototyped behaviour, not just screens</h3><p>I built both as high-fidelity interactive prototypes in <strong>Claude Design</strong>, closely reproducing Altrata’s design system and live search behaviour. The sessions could focus on creating, understanding, modifying and recovering a query, rather than prototype limitations.</p><Copy from={139} /><blockquote>{source[142]}</blockquote></>
      break
    case 'round-1':
      content = <><p>I started internally to identify structural, technical and usability risks while they were inexpensive to change. This round was not intended to validate the final direction with clients.</p><Method people="9 · Engineering, Data, Product and experienced colleagues" task="Realistic search scenarios supplied to everyone" design="Both concepts per participant; counterbalanced order" /><div className="ae-score-grid"><div><Label>Ease of completion · SEQ</Label><strong>5.1 <small>/ 7 · A</small></strong><strong>5.2 <small>/ 7 · B</small></strong></div><div><Label>Overall preference</Label><strong>4 <small>for A</small></strong><strong>5 <small>for B</small></strong></div><p>The scores were close and directional. <strong>The friction was more informative than preference.</strong> We also assessed confidence that the query matched the intended criteria.</p></div><h3>What the two models revealed</h3><Table headings={['Finding', 'Why it mattered', 'Next iteration']} rows={[
      ...
      ]} /><h3>Less noise, the same expressive power</h3><Schematic kind="logic" /><p>In this iteration, module and filter connectors appeared only when both sides contained active criteria. Value-level operators appeared after multiple values were selected.</p><ImageSpace after={171} /><div className="ae-split"><section><Label>Question kept open</Label><h3>Was cross-module OR useful?</h3><p>Internal participants saw limited value, but the builder made its scope clearer. I kept it in Concept A for the next round rather than turning an internal observation into a product rule.</p></section><section><Label>Research adjustment</Label><h3>Measure confidence, not broad usefulness.</h3><p>A concept could feel powerful yet remain difficult to operate. From the next round, we focused on ease of building and confidence that the resulting query represented the intended logic.</p></section></div><Takeaway>{source[179]}</Takeaway></>
      break
    case 'round-2':
      content = <><p>Round one tested whether the concepts were coherent. Round two tested whether they matched <strong>the searches clients actually needed to perform</strong>. I sampled across verticals and technical confidence, asking colleagues to bring complex real client requests.</p><Method people="8 · Commercial, Account Management and Client Success" task="A complex client search each participant brought" design="Both concepts per participant; counterbalanced order" /><div className="ae-round-map"><span>01 Simplify relationships</span><span>02 Make queries readable</span><span>03 Improve discovery</span><span>04 Validate the domain model</span></div><h3>01 · Move relationships to where they can be understood</h3><p>OR within a criterion was easy to explain: CEO OR CTO, New York OR Washington. OR between unrelated modules was harder: participants struggled to offer convincing client scenarios or identify the connector’s scope.</p><Table headings={['Evidence', 'Design response', 'Reasoning']} rows={[
    ...
    case 'round-3':
      ...
      ]} /><Copy from={330} /><div className="ae-research-note"><Label>The value of the research sequence</Label><p>Customer-facing teams anticipated the need for a familiar panel, unambiguous connectors and a readable query. They did not replace client research; they removed predictable problems before limited client access was used to validate the direction. Remaining issues were mostly minor usability or prototype-fidelity problems.</p></div></>
      break
    case 'direction':
      content = <><p>Task completion, benchmark speed, confidence and exploratory search behaviour all pointed to Concept B.</p><div className="ae-convergence"><section><Label>From Concept B</Label><h3>Familiarity + discovery</h3><p>Keep the panel, support browsing and let users shape their search progressively.</p></section><span aria-hidden="true">+</span><section><Label>Learned from Concept A</Label><h3>Visibility + control</h3><p>Make query relationships readable, verifiable and directly editable.</p></section></div><Takeaway>{source[352]}</Takeaway><Copy from={353} /></>
      break
    case 'impact':
    ...
    case 'reflection':
      ...
      ].map(([n,title,learning,next])=><section key={n}><span>{n}</span><div><h3>{title}</h3><p>{learning}</p><p><strong>Next time:</strong> {next}</p></div></section>)}</div><blockquote>{source[472]}</blockquote></>
      break
    default:
      return <>{children}</>
  }
