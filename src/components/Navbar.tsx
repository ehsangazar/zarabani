      ...
      <div className="mx-auto flex h-20 max-w-7xl items-center justify-between px-6 lg:px-8">
        <Link to="/" className="group flex items-center space-x-4">
          ...
          <div className="flex flex-col">
            <span className="text-xl lg:text-2xl font-extrabold text-primary-dark group-hover:text-primary transition-colors">
              Zara Bani
            </span>
            <span className="text-xs lg:text-sm font-semibold uppercase tracking-wider text-gradient">
              Product & UX Designer
            </span>
          </div>
        </Link>
        ...
        <div className="hidden items-center space-x-1.5 md:flex">
          {navItems.map((item) => {
            ...
            return (
              ...
              >
                {isActive && (
                  <span className="absolute inset-0 rounded-xl bg-primary shadow-lg shadow-primary/30"></span>
                )}
                {!isActive && (
        ...
        <button
          ...
          className="group relative rounded-xl border border-primary/20 bg-white/50 p-2.5 backdrop-blur-sm hover:border-primary/40 hover:bg-white/80 md:hidden"
        >
          <svg
        ...
        <div className="glass-dark border-t border-white/10 shadow-2xl md:hidden">
          <div className="space-y-1.5 px-4 py-4">
            {navItems.map((item) => {
              ...
              return (
                <Link
                  ...
                  className={`block rounded-xl px-4 py-3.5 text-sm font-bold ${
                    isActive
            ...
            <a
              ...
              rel="noopener noreferrer"
              onClick={() => setIsOpen(false)}
              className="block rounded-xl bg-accent px-4 py-3.5 text-center text-sm font-bold text-white shadow-lg shadow-accent/30"
            >
              Connect on LinkedIn
            </a>
          </div>
